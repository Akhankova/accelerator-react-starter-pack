...
import { Comments, SmallCardCart } from '../../types/cards';
...
function ProductCard(props: Props): JSX.Element {
  const { name, rating, previewImg, price, id, comments, vendorCode, stringCount, type, count } = props;
  ...
  useEffect(() => {
    ...
    if (card[0]) { setInCart(true); }
  }, [cardsCart, name]);

  useEffect(() => {
    if (isBookingModalOpened || isModalSuccessAddOpened) {
      document.body.style.overflow = 'hidden';
      document.body.setAttribute('aria-hidden', 'true');
    } else { document.body.style.overflow = 'scroll'; }
  }, [isBookingModalOpened, isModalSuccessAddOpened]);

  return (
    <div className="product-card">
      {!isBookingModalOpened ? null : <ModalCart onOpen={() => setIsModalSuccessAddOpened(true)} onClose={() => setIsBookingModalOpened(false)} card={{ name, rating, previewImg, price, id, comments, vendorCode, stringCount, type, count } as SmallCardCart} />}
