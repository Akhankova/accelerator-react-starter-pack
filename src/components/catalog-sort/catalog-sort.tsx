...
function CatalogSort(): JSX.Element {
  ...
  //const styleButtomDesc = isSortOrder !== `${Sort.Descending}` ? {borderTopColor: '#a1a1a1', borderBottomColor: 'rgb(100, 100, 100)'} : {borderBottomColor: '#585757'};
  ...
  return (
    <div className="catalog-sort">
      ...
      <div className="catalog-sort__order">
        ...
        <button className={`catalog-sort__order-button catalog-sort__order-button--down ${isSortOrder === `${Sort.Descending}` ? 'catalog-sort__order-button--active' : ''}`} aria-label="По убыванию" onClick={(event) => dispatchAction(setOrder(event.currentTarget.value))} tabIndex={isSortOrder === `${Sort.Descending}` ? -1 : 0} value='По убыванию'></button>
      </div>
    </div>
  );
}
